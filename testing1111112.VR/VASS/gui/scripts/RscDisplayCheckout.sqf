...
switch _mode do {
	...
	case "refresh":{
		...
		//--- Funds
		_fundes = with missionnamespace do {["getMoney",[_center]] call TER_fnc_VASShandler};
		_fundesText =  [_fundes, "left"] call _fncTparams;
		//--- Costs
		...
		_stxtMoney ctrlSetStructuredText composeText [parsetext _topLine,lineBreak,parsetext _fundesText,parsetext _costText,parsetext _diffText];
		//--- Button buy
		_btnBuy = _grpCheckout controlsGroupCtrl IDC_RSCDISPLAYCHECKOUT_BTNBUY;
		if (_fundes >= _cost) then {
